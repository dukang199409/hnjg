 ...
 */
@Controller
@RequestMapping("/wxJsAllWasteManager")
public class WxJsAllWasteController {
	...
	@ResponseBody
	public BSLResult listWaste(QueryCriteria queryCriteria){
		BSLResult result = null;
		if(StringUtils.isBlank(queryCriteria.getPage())) {
			result =  BSLResult.build(400, "页码不能为空");
		}else if(StringUtils.isBlank(queryCriteria.getRows())) {
			...
			result = wxJsAllProdsService.getWxJsAllWastes(queryCriteria);
		}
		return result;
	}
