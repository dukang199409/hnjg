...
/**
 * @author 杜康
 * 外协厂废品销售出库管理Controller
 */
@Controller
@RequestMapping("/saleWasteWx")
public class SaleWasteWxController {
	
	@Autowired
	SaleWasteWxService saleWasteWxService;
	...
	public BSLResult getOutProds(QueryCriteria criteria){
		...
		return saleWasteWxService.getOutWastesService(criteria);
	}
	...
	public BSLResult saleWasteOutPut(String saleSerno,Float prodOutWeight,String inputUser) {
		...
		try {
			return saleWasteWxService.updateSaleWasteOutPut(saleSerno, prodOutWeight, inputUser);
		} catch (Exception e) {
	...
	public BSLResult reBackSaleWaste(String saleSerno,Float prodRebackWeight,String inputUser) {
		...
		try {
			return saleWasteWxService.updateReBackSaleWaste(saleSerno, prodRebackWeight, inputUser);
		} catch (Exception e) {
			DictItemOperation.log.info("===========异常:"+e.getMessage());
			return BSLResult.build(ErrorCodeInfo.错误类型_交易异常,e.getMessage());
		}
